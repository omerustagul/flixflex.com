...
const FFDivider = React.forwardRef<HTMLDivElement, FFDividerProps>(
  ({ label, variant = "default", orientation = "horizontal", className, ...props }, ref) => {
    const lineClass = {
      ...
      purple:   "bg-[rgba(161,52,255,0.4)]",
      gradient: "bg-gradient-to-r from-transparent via-[rgba(161,52,255,0.5)] to-transparent",
    }[variant]
    ...
    if (label) {
      return (
        <div ref={ref} className={cn("flex items-center gap-4", className)} {...props}>
          ...
          <span className="text-[11px] font-medium tracking-[0.1em] uppercase text-[var(--foreground-faint)] shrink-0">
            {label}
          </span>
          <div className={cn("flex-1 h-px", lineClass)} />
        </div>
